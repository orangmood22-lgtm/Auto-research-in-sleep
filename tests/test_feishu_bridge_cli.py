#!/usr/bin/env python3
"""Behavior tests for the ARIS Feishu/Lark bridge CLI wrapper."""
...
class FeishuBridgeCliTest(unittest.TestCase):
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="aris-feishu-cli-"))
    ...
    def _run(self, *args: str, cwd: Path | None = None, env: dict | None = None) -> subprocess.CompletedProcess:
        return subprocess.run(
            [str(ARIS_CLI), "--aris-repo", str(self.framework), *args],
    ...
    def test_feishu_install_dry_run_uses_lark_channel_bridge_package(self) -> None:
        ...
        self.assertIn(f"command: npm i -g --prefix {self.home / '.aris' / 'node'} lark-channel-bridge", npm.stdout)
        self.assertIn(f"bin_dir: {self.home / '.aris' / 'node' / 'bin'}", npm.stdout)

        pnpm = self._run("feishu", "install", "--manager", "pnpm", "--dry-run")
        self.assertEqual(pnpm.returncode, 0, msg=pnpm.stderr)
        self.assertIn(f"command: pnpm add -g --global-bin-dir {self.home / '.aris' / 'node' / 'bin'} lark-channel-bridge", pnpm.stdout)
    ...
    def test_feishu_run_dry_run_defaults_to_codex_profile_and_current_workspace(self) -> None:
        ...
        self.assertIn("--profile aris-codex", result.stdout)
    ...
    def test_feishu_start_can_target_claude_lark_and_app_id(self) -> None:
        ...
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertIn("--profile aris-claude", result.stdout)
    ...
    def test_feishu_bridge_run_sets_user_local_npm_prefix_for_child_auto_installs(self) -> None:
        ...
        self._write_executable(
            ...
            "\n".join(
                [
                    ...
                    "printf 'NPM_CONFIG_PREFIX=%s\\n' \"${NPM_CONFIG_PREFIX:-}\" > \"$ARIS_FAKE_BRIDGE_LOG\"",
                    "printf 'npm_config_prefix=%s\\n' \"${npm_config_prefix:-}\" >> \"$ARIS_FAKE_BRIDGE_LOG\"",
                    "printf 'PATH=%s\\n' \"$PATH\" >> \"$ARIS_FAKE_BRIDGE_LOG\"",
                ]
            )
            + "\n",
        )
        ...
        result = self._run(
            ...
            env=self._env(ARIS_FAKE_BRIDGE_LOG=str(log)),
        )
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        content = log.read_text(encoding="utf-8")
        self.assertIn(f"NPM_CONFIG_PREFIX={self.home / '.aris' / 'node'}", content)
        self.assertIn(f"npm_config_prefix={self.home / '.aris' / 'node'}", content)
        self.assertIn(str(self.home / ".aris" / "node" / "bin"), content)
    ...
    def test_feishu_bridge_no_proxy_clears_proxy_env_for_node_sdk(self) -> None:
        ...
        self._write_executable(
            ...
            "\n".join(
                [
                    "if [[ \"${1:-}\" == \"--version\" ]]; then printf '0.3.1\\n'; exit 0; fi",
                    "printf 'http_proxy=%s\\n' \"${http_proxy:-}\" > \"$ARIS_FAKE_BRIDGE_LOG\"",
                    "printf 'https_proxy=%s\\n' \"${https_proxy:-}\" >> \"$ARIS_FAKE_BRIDGE_LOG\"",
                    "printf 'HTTP_PROXY=%s\\n' \"${HTTP_PROXY:-}\" >> \"$ARIS_FAKE_BRIDGE_LOG\"",
                    "printf 'HTTPS_PROXY=%s\\n' \"${HTTPS_PROXY:-}\" >> \"$ARIS_FAKE_BRIDGE_LOG\"",
                ]
            )
    ...
    def test_feishu_logs_tails_latest_profile_log(self) -> None:
        logs = self.tmp / "lark-home" / "profiles" / "aris-codex" / "logs" / "daemon"
        ...
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertIn(f"logs: {self.tmp / 'lark-home' / 'profiles' / 'aris-codex' / 'logs'}", result.stdout)
