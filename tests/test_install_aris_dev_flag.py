#!/usr/bin/env python3
"""Regression test for install_aris.sh --dev flag (dev framework selection).

Covers:
  install --dev: uses aris-dev/ repo instead of stable
  install --dev --aris-repo: explicit --aris-repo overrides --dev
...
DEV_REPO = REPO_ROOT.parent / "aris-dev"


class DevFlagTest(unittest.TestCase):
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="aris-dev-"))
    ...
    def test_dev_flag_uses_dev_repo(self):
        """--dev should use aris-dev/ repo instead of stable."""
        ...
        ver_file = self.project / ".aris" / "framework-version.txt"
        if ver_file.exists():
    ...
    def test_explicit_aris_repo_overrides_dev(self):
        """Explicit --aris-repo should override --dev."""
        if not DEV_REPO.exists():
            self.skipTest(f"dev repo not found at {DEV_REPO}")
        ...
        if ver_file.exists():
            content = ver_file.read_text().strip()
            self.assertNotEqual(content, "unknown", "explicit --aris-repo should override --dev")
