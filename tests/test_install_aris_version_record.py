#!/usr/bin/env python3
"""Regression test for install_aris.sh framework version recording.

Covers:
  install: records framework version + commit to .aris/ after successful install
  install: writes "unknown" when aris-repo has no .git
...
class VersionRecordTest(unittest.TestCase):
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="aris-version-"))
    ...
    def test_install_records_framework_version(self):
        """After install, .aris/ should contain version and commit files."""
        result = self._run()
        self.assertEqual(result.returncode, 0, msg=result.stderr)

        ver_file = self.project / ".aris" / "framework-version.txt"
        commit_file = self.project / ".aris" / "framework-commit.txt"
    ...
    def test_install_records_unknown_for_repo_without_git(self):
        """If aris-repo has no .git, version files should contain 'unknown'."""
